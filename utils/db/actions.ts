import { reducer } from './../../node_modules/react-hot-toast/src/core/store';
...
try {
const user = await db.insert(Users).values({name,email}).returning().execute()
return user
} catch (error) {
...
export async function getRewardTransactions (userId:number) {
    ...
    try {
        ...
        }).from(Transactions).where(eq(Transactions.userId,userId)).orderBy(desc(Transactions.date)).limit(5).execute()
